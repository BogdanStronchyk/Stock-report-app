...
def _median_pos(vals):
    xs = [float(v) for v in vals if
          v is not None and float(v) > 0 and not (isinstance(v, float) and (math.isnan(v) or math.isinf(v)))]
    if not xs:
        return None
...
def _ni_series_annual(annual_income: pd.DataFrame, n: int = 8):
    if annual_income is None or annual_income.empty:
        return []
    return annual_series(annual_income, ["Net Income", "NetIncome", "Net Income Common Stockholders"], n)
...
def compute_davf(
    ...
    if base_type.startswith("FCF") and len([v for v in fcf_series if v is not None]) >= 4:
        xs = _winsorize([float(v) for v in fcf_series if v is not None])
        if len(xs) >= 4:
            mu = float(np.mean(xs))
            sig = float(np.std(xs))
...
def _fmp_first_record(payload: Any) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(payload, list) and payload:
        if isinstance(payload[0], dict): return payload[0]
    return None
...
def _adj_close(hist: pd.DataFrame) -> pd.Series:
    ...
    if "Adj Close" in hist.columns: return hist["Adj Close"].astype(float)
...
def _history_retry(symbol: str, tkr: yf.Ticker, period: str, interval: str = "1d", tries: int = 3) -> pd.DataFrame:
    ...
    if h2 is not None and not h2.empty: return h2
    return pd.DataFrame()
...
def realized_vol_1y(hist_1y: pd.DataFrame) -> Optional[float]:
    if hist_1y is None or hist_1y.empty: return None
...
def worst_weekly_return_3y(hist_3y: pd.DataFrame) -> Optional[float]:
    if hist_3y is None or hist_3y.empty: return None
    close = _adj_close(hist_3y).dropna()
    if len(close) < 200: return None
    weekly = close.resample("W-FRI").last().pct_change().dropna()
    if weekly.empty: return None
    return float(weekly.min() * 100.0)


def approx_roic_percent(annual_income: pd.DataFrame, annual_bs: pd.DataFrame, tax_rate_assumption: float = 0.21) -> \
Optional[float]:
    ...
    for key in ["EBIT", "Ebit", "Operating Income", "OperatingIncome"]:
        if key in annual_income.index:
            ...
            break
    if ebit is None or pd.isna(ebit): return None
    total_assets = None
    curr_liab = None
...
def zscore_last(series: pd.Series, window: int = 2520) -> Optional[float]:
    ...
    if len(s) < max(100, window // 4): return None
    tail = s.iloc[-window:] if len(s) >= window else s
    mu = float(tail.mean())
    sd = float(tail.std(ddof=0))
    if sd == 0 or math.isnan(sd): return None
...
def stock_nupl_regime_from_z(z: Optional[float]) -> str:
    if z is None or (isinstance(z, float) and (math.isnan(z) or math.isinf(z))):
        return "NA"
    if z <= -2.5: return "Deep Capitulation"
    if z <= -1.5: return "Capitulation"
    if z <= -0.5: return "Stress"
    if z < 0.5: return "Neutral"
    if z < 1.5: return "Optimism"
    return "Euphoria"


def normalize_cycle_proxy(current_price: float, ath: float) -> Optional[float]:
    if ath is None or ath <= 0: return None
...
def compute_metrics_v2(
    ...
    def _cached_json(key: str, fn):
        ...
        try:
            ...
            v2 = None
        if v2 is not None: yf_cache.set_json(key, v2)
        return v2
    ...
    if not use_fmp_effective:
        ...
        if forced and not requested:
            notes[
                "FMP"] = f"enabled=yes (FORCED, mode={fmp_mode_effective}); requests={fmp.request_count}; last_status={fmp.last_status}"
        else:
            notes[
                "FMP"] = f"enabled=yes (mode={fmp_mode_effective}); requests={fmp.request_count}; last_status={fmp.last_status}"
    else:
    ...
    fi_price = _to_float(
        fast_info.get("last_price") or fast_info.get("lastPrice") or fast_info.get("regularMarketPrice"))
    price = float(p5.iloc[-1]) if not p5.empty else fi_price
    if price is None: price = safe_get(info, "currentPrice")
    if price is None: price = safe_get(info, "regularMarketPrice")
    if price is None: price = _fmp_get_num(fmp_data, "quote", "price")

    mcap = safe_get(info, "marketCap")
    if mcap is None: mcap = _to_float(fast_info.get("market_cap") or fast_info.get("marketCap"))
    if mcap is None: mcap = _fmp_get_num(fmp_data, "quote", "marketCap")

    ev = safe_get(info, "enterpriseValue")
    if ev is None: ev = _fmp_get_num(fmp_data, "profile", "enterpriseValue")
    if ev is None: ev = _fmp_get_num(fmp_data, "key_metrics_ttm", "enterpriseValue")
    if ev is None: ev = _fmp_get_num(fmp_data, "quote", "enterpriseValue")
    if ev is None: ev = mcap

    shares_out = safe_get(info, "sharesOutstanding")
    if shares_out is None: shares_out = _to_float(fast_info.get("shares") or fast_info.get("shares_outstanding"))
    if shares_out is None: shares_out = _fmp_get_num(fmp_data, "profile", "sharesOutstanding")
    ...
    ttm_gp = last_n_quarters_sum(q_income, ["Gross Profit", "GrossProfit"], n=4)
    ttm_ebit = last_n_quarters_sum(q_income, ["Operating Income", "OperatingIncome", "EBIT"], n=4)
    ttm_ni = last_n_quarters_sum(q_income, ["Net Income", "NetIncome", "Net Income Common Stockholders"], n=4)
    ...
    # TTM EBITDA (Calculated Manually)
    ttm_da = last_n_quarters_sum(q_cf, ["Depreciation And Amortization", "Depreciation", "DepreciationAndAmortization"],
                                 n=4)
    ttm_ebitda_manual = None
    if ttm_ebit is not None and ttm_da is not None:
        ttm_ebitda_manual = float(ttm_ebit) + abs(float(ttm_da))

    # Balance Sheet Items
    stockholders_equity = None
    if annual_bs is not None and not annual_bs.empty:
        for k in ["Stockholders Equity", "StockholdersEquity", "Total Equity Gross Minority Interest"]:
            if k in annual_bs.index:
                stockholders_equity = _to_float(annual_bs.loc[k].iloc[0])
                break

    # --- VALUATION METRICS (With Manual Fallbacks) ---
    trailing_pe = safe_get(info, "trailingPE")
    if trailing_pe is None and mcap is not None and ttm_ni is not None and ttm_ni > 0:
        trailing_pe = float(mcap) / float(ttm_ni)

    ev_ebitda = safe_get(info, "enterpriseToEbitda")
    if ev_ebitda is None and ev is not None and ttm_ebitda_manual is not None and ttm_ebitda_manual > 0:
        ev_ebitda = float(ev) / float(ttm_ebitda_manual)

    ps = safe_get(info, "priceToSalesTrailing12Months")
    if ps is None: ps = _fmp_get_num(fmp_data, "ratios_ttm", "priceToSalesRatioTTM", "priceToSalesRatio")
    ...
    if ev in (None, 0) or ebit_used is None:
        notes["EV/EBIT"] = "Missing EV or EBIT."
        notes["Earnings Yield (EBIT / EV)"] = "Missing EV or EBIT."
    elif ebit_used <= 0:
        notes["EV/EBIT"] = "EBIT <= 0."
        notes["Earnings Yield (EBIT / EV)"] = "EBIT <= 0."
    else:
        if ttm_rev not in (None, 0) and (float(ebit_used) / float(ttm_rev)) < 0.005:
            notes["EV/EBIT"] = "EBIT margin < 0.5% (unstable)."
        else:
            ev_ebit = float(ev) / float(ebit_used)
            earnings_yield = (1.0 / ev_ebit) * 100.0 if ev_ebit not in (None, 0) else None

    # EV/Gross Profit (Manual)
    ev_gross_profit = None
    if ev is not None and ttm_gp is not None and ttm_gp > 0:
        ev_gross_profit = float(ev) / float(ttm_gp)
    elif ev is not None and annual_income is not None and not annual_income.empty:
        # Fallback to annual
        gp = None
    ...
    # --- PROFITABILITY (With Manual Fallbacks) ---
    ...
    if gross_m_pct is None and ttm_gp is not None and ttm_rev not in (None, 0):
        gross_m_pct = (float(ttm_gp) / float(ttm_rev)) * 100.0

    op_m_pct = op_m * 100 if op_m is not None else None
    if op_m_pct is None and ttm_ebit is not None and ttm_rev not in (None, 0):
        op_m_pct = (float(ttm_ebit) / float(ttm_rev)) * 100.0

    net_m_pct = net_m * 100 if net_m is not None else None
    if net_m_pct is None and ttm_ni is not None and ttm_rev not in (None, 0):
        net_m_pct = (float(ttm_ni) / float(ttm_rev)) * 100.0

    roe_pct = roe * 100 if roe is not None else None
    if roe_pct is None and ttm_ni is not None and stockholders_equity not in (None, 0):
        roe_pct = (float(ttm_ni) / float(stockholders_equity)) * 100.0
    ...
    if beta is None: beta = _fmp_get_num(fmp_data, "profile", "beta")
    ...
    if ebitda is None: ebitda = _fmp_get_num(fmp_data, "key_metrics_ttm", "ebitdaTTM", "ebitda")
    if ebitda is None: ebitda = ttm_ebitda_manual
    ...
    if annual_bs is not None and not annual_bs.empty:
        ...
        assets = None
        for k in ["Cash And Cash Equivalents", "CashAndCashEquivalents"]:
            if k in annual_bs.index: cash = annual_bs.loc[k].iloc[0]; break
        for k in ["Total Assets", "TotalAssets"]:
            if k in annual_bs.index: assets = annual_bs.loc[k].iloc[0]; break
    ...
    rev_cagr = cagr(revs)
    shares = safe_get(info, "sharesOutstanding") or shares_out
    revps_cagr = None
    ...
    avg_dollar_vol = None
    try:
        h3m = _slice_history_days(h10, 110)
        if h3m is not None and not h3m.empty and "Volume" in h3m.columns:
            px = _adj_close(h3m)
            dv = (px * pd.to_numeric(h3m["Volume"], errors="coerce")).dropna()
            if len(dv) > 10: avg_dollar_vol = float(dv.mean())
    except Exception:
        pass

    # NUPL
    vwap10_series = anchored_vwap(p10, h10["Volume"]) if (
                h10 is not None and not h10.empty and "Volume" in h10.columns) else pd.Series(dtype=float)
    ...
    realized_series = turnover_realized_price(p10, h10["Volume"], shares_out) if (
                h10 is not None and not h10.empty and "Volume" in h10.columns) else pd.Series(dtype=float)
    ...
    zsum = sum(weights[k] for k in weights if z_vals[k] is not None)
    composite_z = None
    if zsum > 0:
        composite_z = sum((weights[k] / zsum) * z_vals[k] for k in weights if z_vals[k] is not None)
    nupl_regime = stock_nupl_regime_from_z(composite_z)

    # DAVF
    davf = compute_davf(sector_bucket, price, shares_out, annual_income, annual_cf, annual_bs, roic_pct, nd_ebitda,
                        interest_cov)
    if davf.get("davf_note"): notes["DAVF"] = davf["davf_note"]
    if davf.get("davf_label") != "NA":
        notes["DAVF Downside Protection"] = f"{davf['davf_label']} (MOS={davf.get('davf_mos_pct'):.1f}%)"

    extra_metrics: Dict[str, Any] = {}
    try:
        extra_metrics, extra_notes = compute_value_matrix_extras(
            info=info, fmp_bundle=fmp_data, price=price, market_cap=mcap, enterprise_value=ev,
            annual_income=annual_income, annual_cashflow=annual_cf, annual_balance_sheet=annual_bs,
            quarterly_cashflow=q_cf
        )
        if extra_notes: notes.update(extra_notes)
    except Exception as e:
        notes["Value Matrix Extras"] = f"Failed: {e}"
    ...
    metrics = {
        ...
        "Beta (5Y)": beta,
        "Short Interest (% float)": (safe_get(info, "shortPercentOfFloat") * 100.0) if safe_get(info,
                                                                                                "shortPercentOfFloat") else None,
        "Days to Cover": safe_get(info, "shortRatio"),
        "Avg Daily $ Volume (3M)": avg_dollar_vol,
        "Market Cap": mcap,
        "Worst Weekly Return (3Y)": worst_week_3y,
        "NUPL10Y": (price - twap(p10)) / price if (price and twap(p10)) else None,
        "NUPL5Y": (price - twap(p5)) / price if (price and twap(p5)) else None,
    ...
    metrics["__yf_bundle__"] = {
        "info": info, "q_income": q_income, "q_cf": q_cf, "annual_bs": annual_bs,
        "h10": h10, "h1y": h1y, "h2y": h2y,
    }
