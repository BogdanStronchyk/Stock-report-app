...
def compute_metrics_v2(
    ...
    *,
    ...
    if fmp_mode and fmp_mode != "full":
        fmp_mode_effective = fmp_mode
    elif env_mode:
        fmp_mode_effective = env_mode
    else:
        fmp_mode_effective = fmp_mode or "full"
    ...
    if not use_fmp_effective:
        ...
        if forced and not requested:
            notes["FMP"] = f"enabled=yes (FORCED, mode={fmp_mode_effective}); req={fmp.request_count}"
        else:
            notes["FMP"] = f"enabled=yes (mode={fmp_mode_effective}); req={fmp.request_count}"
    else:
    ...
    if ps is None and mcap is not None and ttm_rev not in (None, 0): ps = float(mcap) / float(ttm_rev)
    ...
    if shares is None: shares = shares_out
