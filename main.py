...
def main():
    ...
    # UI FEEDBACK: Start Progress Bar Early for Resolution phase
    pwin = ProgressWindow(100, title=f"Scraping ({rule_mode})...")
    pwin.step(main_text="Preparing data...", sub_text="Resolving symbols and loading universes...")
    ...
    if not tickers: pwin.close(); return

    # Reset Progress bar to accurate ticker count
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for fut in as_completed(futures):
            ...
            pwin.step(sub_text=f"Analyzing {t_sym}...")

    # RULE TRUNCATION LAYER
    pwin.step(main_text="Filtering results...", sub_text=f"Applying {rule_mode} rules...")
    strong_buy_list = []
    for t in tickers:
        if t in metrics_map and t in reversal_map:
            ...
            if all(s is not None and s >= target_threshold for s in
                   cat_scores.values()) and r_score >= target_threshold:
                strong_buy_list.append(t)

    if not strong_buy_list:
        pwin.close();
        print(f"No Strong Buys found for {rule_mode} set.");
        return

    out_file = os.path.join(out_dir, f"Strong_Buys_{rule_mode}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
    pwin.step(main_text="Writing Report...", sub_text="Formatting final Excel file...")
