...
def _limits_text(th: Optional[Dict[str, Any]]) -> str:
    if not th: return ""
    return " | ".join([f"{p}:{th.get(k)}" for k, p in [("green_txt", "G"), ("yellow_txt", "Y"), ("red_txt", "R")] if th.get(k)])
...
def final_recommendation_banner(cat_scores: Dict[str, Optional[float]], reversal_total: Optional[float], threshold: float) -> Tuple[str, Any]:
    ...
    return ("❌ AVOID — Significant Fundamental Risks", FILL_RED) if any(s < 30 for s in scores.values()) else ("⚠ HOLD / NEUTRAL", FILL_YELLOW)
...
def _write_reversal_block(ws, start_row: int, title: str, symbols: Dict[str, str], details: Dict[str, Tuple[int, str]], score: Optional[float]) -> int:
    ws[f"A{start_row}"] = title; ws.merge_cells(start_row=start_row, start_column=1, end_row=start_row, end_column=6); start_row += 1
    for i, h in enumerate(["Condition", "Score", "Details"]):
        c = ws.cell(start_row, i + 1); c.value = h; c.fill = FILL_HDR; c.font = FONT_HDR
    ws.cell(start_row, 4).value = f"Score: {float(score):.1f}%" if score is not None else "Score: NA"; start_row += 1
    for cond, sym in symbols.items():
        ws.cell(start_row, 1).value = cond; ws.cell(start_row, 2).value = sym
        det = details.get(cond); ws.cell(start_row, 3).value = det[1] if isinstance(det, (list, tuple)) else str(det); start_row += 1
    return start_row + 1
...
def create_report_workbook(tickers: List[str], thresholds: Dict[str, Dict[str, Dict[str, Any]]],
    ...
    ws_sum.append(["Ticker", "Sector", "Avg Fund Score", "Reversal Score", "Recommendation", "Valuation", "Quality", "Safety", "Growth", "Risk", "Coverage %"])
    ...
    for t in tickers:
        m = metrics_by_ticker.get(t, {}); revpack = _normalize_reversal_pack(reversal_by_ticker.get(t, {}))
        bucket = m.get("Sector Bucket", "Default (All)")
        ws = wb.create_sheet(t); ws["A1"] = f"{t} — {bucket}"; ws.merge_cells("A1:C1"); ws["A1"].font = Font(size=14, bold=True)
        cat_scores, cat_coverages, row = {}, {}, 3
        for cat_sheet, cat_display in category_maps.items():
            ws.cell(row, 1, cat_display).font = Font(bold=True, size=12); ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=6); row += 1
            for i, h in enumerate(["Metric", "Value", "Rating", "Mode", "Limits", "Notes"]):
                c = ws.cell(row, i + 1); c.value = h; c.fill = FILL_HDR; c.font = FONT_HDR
            row += 1
            ratings, weights = {}, {}
            for metric in thresholds.get(cat_sheet, {}):
                if metric not in WHITELIST: continue
                val = m.get(metric); th = get_threshold_set(thresholds, cat_sheet, metric, bucket)
                ...
                ws.cell(row, 1, metric); c = ws.cell(row, 2, val); _apply_metric_value_format(c, metric, val)
                ws.cell(row, 3, rating).fill = fill; ws.cell(row, 4, bucket); ws.cell(row, 5, _limits_text(th)); ws.cell(row, 6, th.get("notes", "")); row += 1
            ...
            ws.cell(row, 1, f"{cat_display} Adjusted: {adj:.1f}%" if adj is not None else "NA").font = Font(bold=True); row += 2
        ...
        ws["D1"] = "Avg Fund Score"; ws["E1"] = avg_f; ws["E1"].fill = band_fill(avg_f)
        ws["D2"] = "Reversal Score"; ws["E2"] = rev_total; ws["E2"].fill = reversal_fill(rev_total)
        ws["A2"] = "Recommendation"; ws["B2"] = rec_txt; ws["B2"].fill = rec_fill
        avg_cov = sum(cat_coverages.values()) / len(cat_coverages) if cat_coverages else 0
        ws_sum.append([t, bucket, avg_f, rev_total, rec_txt, cat_scores.get("Valuation"), cat_scores.get("Quality"), cat_scores.get("Safety"), cat_scores.get("Growth"), cat_scores.get("Risk"), avg_cov])
        row = _write_reversal_block(ws, row, "Fundamental Turnaround", revpack.get("fund_symbols", {}), revpack.get("fund_details", {}), revpack.get("fund_score_pct"))
        row = _write_reversal_block(ws, row, "Technical Confirmation", revpack.get("tech_symbols", {}), revpack.get("tech_details", {}), revpack.get("tech_score_pct"))
    wb.save(out_path)